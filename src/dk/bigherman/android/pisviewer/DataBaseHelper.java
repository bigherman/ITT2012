...
{		 
	    ...
	    */
		...
		public ArrayList<String> listCountries()
		{
			...
			Cursor myCursor = myDataBase.rawQuery("SELECT DISTINCT country FROM airfields ORDER BY country", null);
					
			myCursor.moveToNext();
			
			while (!myCursor.isAfterLast()) 
			{
				countries.add(myCursor.getString(0));
				myCursor.moveToNext();
			}
		...
		public ArrayList<Airfield> listAirfieldsInCountry(String countryName)
		{
			ArrayList<Airfield> airfields = new ArrayList<Airfield>();
			
			Cursor myCursor = myDataBase.rawQuery("SELECT icao, lat, long, name FROM airfields WHERE country ='" + countryName +"' ORDER BY name", null);
			
			myCursor.moveToNext();
			
			while (!myCursor.isAfterLast()) 
			{
				Airfield airfield = new Airfield(myCursor.getString(0), myCursor.getDouble(1), myCursor.getDouble(2), myCursor.getString(3));
